...
describe('weight conversion and validation', () => {
  test('stores full-precision kg and lb values', () => {
    expect(convertWeight(100, 'kg')).toEqual({
      weightKg: 100,
      weightLb: 100 * POUNDS_PER_KILOGRAM,
    });
  ...
  test('accepts zero weight and zero TUT', () => {
    expect(
      validateSetInput(
        { kind: 'strength', reps: 1, inputWeight: 0, inputUnit: 'lb', tutSeconds: 0 },
        'machine',
      ),
    ).toBeNull();
  });
  ...
  test('validates cardio metrics and rejects strength fields for cardio', () => {
    ...
    expect(validateSetInput({ kind: 'calories', calories: 1.5 }, 'cardio')).toContain(
      'whole number',
    );
    expect(
      validateSetInput(
        { kind: 'strength', reps: 8, inputWeight: 0, inputUnit: 'kg', tutSeconds: 0 },
        'cardio',
      ),
    ).toContain('duration or calories');
  });
});
